...
def p_statement_list_sep_comma(p):
    ...
    p[0] = StatementListNode(p.lineno(1), p.clauseno(1), [p[1], p[3]])
...
def p_statement_alicespoke(p):
    ...
    p[0] = SpokeNode(p.lineno(1), p.clauseno(1), p[1])
...
def p_statement_wasa(p):
    ...
    p[0] = DeclarationNode(p.lineno(1), p.clauseno(1), [p[1], p[4]])

def p_statement_became(p):
    'statement : ID ASSIGNMENT expression'
    p[0] = AssignmentNode(p.lineno(1), p.clauseno(1), [p[1], p[3]])
...
def p_type_number(p):
    'type : TYPE_NUMBER'
    p[0] = NumberTypeNode(p.lineno(1), p.clauseno(1))

def p_type_letter(p):
    'type : TYPE_LETTER'
    p[0] = LetterTypeNode(p.lineno(1), p.clauseno(1))

def p_expression_not(p):
    'expression : B_NOT expression'
    p[0] = UnaryNode(n.UNARY_OP, p.lineno(2), p.clauseno(2), p[1], p[2])

def p_expression_drank(p):
    'expression : ID DECREMENT'
    idNode = IDNode(p.lineno(1), p.clauseno(1), p[1])
    p[0] = UnaryNode(n.UNARY_OP, p.lineno(2), p.clauseno(2), p[2], idNode)
...
def p_expression_or(p):
    ...
    p[0] = BinaryNode(p.lineno(1), p.clauseno(1), p[2], [p[1],p[3]])
...
def p_factor_number(p):
    'factor : NUMBER'
    p[0] = NumberNode(p.lineno(1), p.clauseno(1), p[1])

def p_factor_letter(p):
    'factor : LETTER'
    p[0] = LetterNode(p.lineno(1), p.clauseno(1), p[1])

def p_factor_id(p):
    'factor : ID'
    p[0] = IDNode(p.lineno(1), p.clauseno(1), p[1])
