...
def p_statement_array_assign(p):
    'statement : ID expression ARRAY_PIECE ASSIGNMENT expression'
    p[0] = ASTNodes.ArrayAssignment(p.lineno(1), p.clauseno(1), p[2])

#TODO: Work out Node format, should be logical expression?
...
def p_statement_if_general(p):
    # if
    'statement    : IF_PERHAPS expression_logical IF_SO statement_list ALICE WAS IF_UNSURE IF_WHICH'
    
    # else if
    'statement    : IF_PERHAPS expression_logical IF_SO statement_list IF_OR IF_MAYBE expression_logical IF_SO statement_list ALICE WAS IF_UNSURE IF_WHICH'
    
    # else
    'statement    : IF_PERHAPS expression_logical IF_SO statement_list IF_OR statement_list ALICE WAS IF_UNSURE IF_WHICH'

def p_statement_if_either(p):
    # if (either = 2 options only)
    'statement    : IF_EITHER expression IF_SO statement_list ALICE WAS IF_UNSURE IF_WHICH'



def p_statement_modifier_function(p):
...
def p_statement_pass_to_modifier_function(p):
    'statement : ID FUNCTION_WENT FUNCTION_THROUGH ID'

# Pass variable to modifier function
def p_statement__function(p):
'statement : FUNCTION_THE FUNCTION_ROOM ID (ARGUMENTS) FUNCTION_CONTAINED DEC_A type statement_list ALICE RETURN_FOUND expression'
...
#Not sure about this, might cause loads of reduce conflicts?
